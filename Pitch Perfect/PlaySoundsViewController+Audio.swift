    ...
    func playSound(rate rate: Float? = nil, pitch: Float? = nil, echo: Bool = false, reverb: Bool = false) {
        ...
        audioPlayerNode.scheduleFile(audioFile, atTime: nil) {
            ...
            if let lastRenderTime = self.audioPlayerNode.lastRenderTime, let playerTime = self.audioPlayerNode.playerTimeForNodeTime(lastRenderTime) {
                ...
                if let rate = rate {
                    delayInSeconds = Double(self.audioFile.length) / Double(self.audioFile.processingFormat.sampleRate) // Double(rate)
                } else {
                    delayInSeconds = Double(self.audioFile.length) / Double(self.audioFile.processingFormat.sampleRate)
                }
            }
            
            // schedule a stop timer for when audio finishes playing
            print(Double(self.audioFile.length) / Double(self.audioFile.processingFormat.sampleRate), "second")
            print(delayInSeconds, "second")
